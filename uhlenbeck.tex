\documentclass[letterpaper,11pt]{amsart}%{article}%
...
%\usepackage{fancyhdr}
%\pagestyle{fancy}
%\lhead{Uhlenbeck Compactification}
%\rhead{}
%\chead{Tuomas Tajakka}
%\cfoot{}
%\rfoot{}
...
%{\color{red} Openness and boundedness??}
...
\begin{align*}
    ...
    % & = -\int_X \left(1 - (\be + i\al) H + \frac{1}{2}(\be^2 + 2 i \al \be - \al^2) H^2\right)\cdot \\
    % & \quad (\ch^B_0(E) + \ch^B_1(E) + \ch^B_2(E)) \\
    % & = - \ch^B_2(E) + (\be + i\al) H\cdot \ch^B_1(E) - \frac{1}{2}(\be^2 + 2 i \al \be - \al^2)H^2 \ch^B_0(E) \\
...
% We are particularly interested in understanding how stability changes upon crossing the vertical wall from one unbounded region to the other.
\begin{center}
\begin{tikzpicture}

%\fill[black!10!white] (-4,0) -- (-3.1,0) arc (180:0:2) -- (1,0) -- (1,3.5) -- (-4,3.5);
...
In this subsection we classify stable and semistable objects on the vertical wall %{\color{red} and in the left unbounded chamber (and maybe right chamber also?)}. 
...
%\subsection{Stability left of the vertical wall}
%In this section we recall the fact that Bridgeland semistability "at the large volume limit" left of the vertical wall coincides with Gieseker stability.
...
%{\color{red} Why does $\det$ respect numerical equivalence? This means that $\la_\sE$ induces a map $\Kn(X) \to N^1(S)$. Answer: if $S$ is proper, it follows from the projection formula that 
%\[ K(X) \to K(S), \quad v \mapsto R p_* (\sE \otimes q^* v) \]
%respects numerical equivalence, and it follows from the Riemann-Roch theorem for curves that
%\[ K(C) \xrightarrow{\det} \Pic(C) \]
%sends numerically trivial classes to degree 0 line bundles. I don't know if this fact is needed here though.
%}
...
%%%%%% Below is an earlier version with the unnecessary "finite type $\C$-scheme $S$"
...
\[ \cdots \to \Hh^i(C, G) \to \Hh^i(C, \sH^1(G)[-1]) \to \Hh^{i+1}(C, \sH^0(G)) \to \cdots. \] % \to \Hh^i(C, \sH^0(G))
...
\begin{center}
    ...
    & X & C \\};
    \path[->] 
    (m-1-2) edge node[auto] {$ p_X $} (m-1-1)
    (m-1-3) edge node[auto] {$ j $} (m-1-2)
    (m-1-3) edge[bend right] node[auto,swap] {$ p_C $} (m-1-1)
    (m-1-2) edge node[auto] {$ q_X $} (m-2-2)
    (m-1-3) edge node[auto,swap] {$ q_C $} (m-2-3)
    (m-2-3) edge node[auto,swap] {$ i $} (m-2-2)
    ;        
    \end{tikzpicture}
\end{center}
...
%\subsection{Random thought (to be removed later (unless it works out))}
%It would be amazing to prove projectivity of the Gieseker moduli space without GIT, using Bridgeland stability instead. Here could be an approach. Let $v \in \Kn(X)$ have positive rank, and let $\si_+, \si_0$ be stability conditions in the Gieseker chamber and on the vertical wall, respectively. The inclusion of stacks $\sM^{\si_+}(v) \subs \sM^{\si_0}(v)$ induces a map on $f: M^{\si_+}(v) \to M^{\si_0}(v)$ on good moduli spaces. Since we already know $M^{\si_0}(v)$ is projective, to show that $M^{\si_+}(v)$ is projective, it should be enough to find a line bundle on $M^{\si_+}(v)$ that is ample on the fibers of $f$. This line should be none other than $\sL_{\si_+}$.

%So what are the fibers? Hopefully some projective spaces parameterizing iterated extensions. For example, consider a point $[F_1 \oplus F_2] \in M^{\si_0}(v)$, where $F_1$ and $F_2$ are $\mu$-stable vector bundles of equal slope but $p(F_2) > p(F_2)$, where $p$ denotes some suitable twisted Hilbert polynomial. Now $f^{-1}([F_1 \oplus F_2])$ should parameterize sheaves $E$ that are extensions of $F_2$ by $F_1$.

%Let $E = \Ext^1(F_2, F_1)$ and consider the projective space $S = \p(E)$ with projections $p: S \times X \to S$ and $q: S \times X \to X$. By \cite[{Example 2.1.12}]{HL}, there is a universal extension
%\[ 0 \to p^* \Oh_S(1) \otimes q^* F_1 \to \sE \to p^* F_2 \to 0. \]
%The sheaf $\sE$ is a family of $\si_+$-stable objects, and also induces a Donaldson morphism $\la_\sE: K(X) \to \Pic S$. Let $u_+ \in K(X)$ be the class giving the Bayer-Macri line bundle on $\sM^{\si_+}(v)$. Now
%\begin{align*}
%    \la_\sE(u_+) & = \la_{q^* F_2}(u_+) \otimes \la_{p^*\Oh_S(1) \otimes q^* F_1}(u_+) \\
%    & = \la_{q^* F_2}(u_+) \otimes \la_{q^* F_1}(u_+) \otimes \Oh_S(1)^{\chi(F_1 \otimes u_+)} \\
%    & = \Oh_S(1)^{\chi(F_1 \otimes u_+)}
%\end{align*}
%because for any locally free sheaves $F$ and $G$ on $X$,
%\[ \la_{q^*F}(G) \otimes \la_{q^*F}(G) = \la_{q^*F}(G) = \det R p_* q^*(F \otimes G) = \det \Oh_S \otimes R \Ga(X, F \otimes G) = \Oh_S. \]
%But $u_+$ by definition satisfies the equation
%\[ \chi(F_1 \otimes u_+) = \im\left(-\frac{Z_{\si_+}(F_1)}{Z_{\si_+(v)}}\right). \]
%Since $\arg Z_{\si_+}(F_1) < \arg Z_{\si_+}(v)$, we have $\chi(F_1 \otimes u_+) > 0$, so the line bundle $\la_\sE(u_+)$ is ample on $S$.



%\begin{lem}\label{constantmapcurvetoAS}
%    If $C \to X$ is a map of algebraic spaces with (some conditions) such that for every map $\Spec \C \to C$, the induced map $\Spec \C \to X$ represents the same point in $|X|$, then $C \to X$ factors through $\Spec \C$.
%\end{lem}
%\begin{proof}
%    {\color{red} Do it.}
%\end{proof}
...
\bibliography{bibliography_copy}
