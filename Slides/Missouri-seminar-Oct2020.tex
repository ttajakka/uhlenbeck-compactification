...
\begin{columns}[t]
    ...
    \begin{itemize}
        ...
        M^\mu_{r, d} & \text{coarse moduli space} \\};
        \path[->]
...
\begin{columns}[t]
    ...
    \begin{itemize}
        ...
        \end{tikzpicture}
        \end{center}
        \item<5-> For $F \in \Coh(C)$ locally free, set
...
\begin{frame}[fragile]{Dim 2: Gieseker and Uhlenbeck moduli spaces}
    ...
    \begin{columns}[t]
        ...
        \begin{itemize}
            ...
            ;
            \path[->]
            (m-1-1) edge node[auto] {$ _\text{cms} $} (m-2-1)
            (m-1-2) edge node[auto] {$ _\text{not cms} $} (m-2-2)
...
\begin{frame}{Dim 2: Bridgeland stability}
    \begin{columns}[t]
        ...
        \end{tikzpicture}
        \end{center}
            \item<13-> \textbf{Thm} (T, A--HL--H): Moduli stack $\sM^\si_v$ is algebraic, of finite type, universally closed, coarse moduli space $M^\si_v$ is a \textit{proper algebraic space}.
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Dim 2: Projectivity on vertical wall \& Uhlenbeck}
    \begin{columns}[t]
        ...
        \begin{itemize}
            ...
            (m-1-3) edge node[auto,swap] {$ _\text{cms} $} (m-2-3)
            (m-2-1) edge node[auto] {$ $} (m-2-2)
